...
public class LockwiseIT {
    private AvailableMetadata lockwise;

    @Before
    public void setUp() throws Exception {
        final GithubConsumer githubConsumer = new GithubConsumer(new ApiConsumer());
        lockwise = new Lockwise(githubConsumer).call();
    }

    @Test
    public void isDownloadUrlAvailable() throws IOException {
        final HttpsURLConnection connection = (HttpsURLConnection) lockwise.getDownloadUrl().openConnection();
    ...
    public void isAppUpToDate() throws ParserConfigurationException, SAXException, IOException {
        ...
        final String actualTitle = String.format("Firefox Lockwise %s by Mozilla", lockwise.getReleaseId().getValueAsString());
